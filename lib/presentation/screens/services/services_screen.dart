...
class _ServicesScreenState extends State<ServicesScreen> {
  ...
  Widget _buildServiceItem(Service service, AppColors colors) {
    ...
    final finLocal = service.fin?.toLocal();

    // Formater l'heure de début
    final startTime = '${debutLocal.hour.toString().padLeft(2, '0')}:${debutLocal.minute.toString().padLeft(2, '0')}';

    // Formater l'heure de fin si elle existe
    String? endTime;
    if (finLocal != null) {
      endTime = '${finLocal.hour.toString().padLeft(2, '0')}:${finLocal.minute.toString().padLeft(2, '0')}';
    }

    // Couleur en fonction du type
    final iconColor = isBreak ? colors.warning : colors.success;
